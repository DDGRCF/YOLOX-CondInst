...
def apply_affine_to_polygons(polygons, target_size, M, scale):
    num_polygons = len(polygons)
    # warp corner points
    twidth, theight = target_size
    polygons_points = np.ones((num_polygons, 3))
    polygons_points[:, :2] = polygons[:, :2]
    polygons_points = polygons_points @ M.T
    np.clip(polygons_points[:, 0], 0, twidth, out=polygons_points[:, 0])
    np.clip(polygons_points[:, 0], 0, twidth, out=polygons_points[:, 0])
    targets = np.hstack((polygons_points[:, :2], polygons[:, 2:]))
...
def _mirror(image, boxes, prob=0.5, masks=None):
...
class TrainTransform:
    def __init__(self, max_labels=50, flip_prob=0.5, hsv_prob=1.0, with_mask=True):
    ...
    def __call__(self, image, bbox_targets, input_dim, mask_targets=None):
        ...
        masks_o = mask_targets.copy() if self.with_mask else None
        ...
        image_t, boxes, masks = _mirror(image, boxes, self.flip_prob, masks=mask_targets)
        ...
        elif self.with_mask and len(boxes_t):
            keep_inds = np.nonzero(filter_b)[0]
            masks = preproc(masks, (input_dim[0], input_dim[1], num_targets), None, 0.)[0]
            masks_t = self._mask_filter(masks, keep_inds, is_tolist=False) # to List
...
class ValTransform:
    ...
    def __call__(self, img, res, input_size, masks):
        ...
        return img, np.zeros((1, 5)), np.zeros((img.shape[0], img.shape[1], 1), np.uint8)
